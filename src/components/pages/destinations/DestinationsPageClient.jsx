...
export default function DestinationsPageClient() { 
  ...
  const totalPages = useMemo(() => {
    ...
    return Math.ceil(sortedDestinations.length / ITEMS_PER_PAGE);
  }, [sortedDestinations]);
  ...
  const currentPage = useMemo(() => {
    ...
    return parsedPage;
  }, [isMounted, nextSearchParams, totalPages]);

  const paginatedDestinations = useMemo(() => {
    if (!isMounted || sortedDestinations.length === 0) {
      return [];
    }
    const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
    return sortedDestinations.slice(startIndex, startIndex + ITEMS_PER_PAGE);
  }, [isMounted, sortedDestinations, currentPage]);
